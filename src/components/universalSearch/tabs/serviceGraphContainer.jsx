...
export default class ServiceGraphContainer extends React.Component {
    ...
    componentDidMount() {
        import(/* webpackChunkName: "serviceGraph", webpackPreload: true */ '../../serviceGraph/serviceGraphResults')
        .then((mod) => {
            this.setState({ServiceGraphResults: mod.default});
        });
    }
    ...
    render() {
        const ServiceGraphResults = this.state.ServiceGraphResults;
            ...
            <section className="div">
                        ...
                        <ul className="nav nav-tabs">
                            {
                                this.props.store.graphs.map(
                                    (graph, index) => (
                                        <li key={Math.random()} className={this.state.tabSelected === (index + 1) ? 'active ' : ''}>
                ...
                { this.props.store.promiseState && this.props.store.promiseState.case({
                    ...
                    fulfilled: () => ((this.props.store.graphs && this.props.store.graphs.length)
                        ? ServiceGraphResults && <ServiceGraphResults serviceGraph={this.props.store.graphs[this.state.tabSelected - 1]} history={this.props.history} />
                        : <Error />)
                })
                }
                </div>
