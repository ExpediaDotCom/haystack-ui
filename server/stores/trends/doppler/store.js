...
const operationCountTemplateName = config.stores.trends.operationCountTemplateName;
const operationDurationTemplateName = config.stores.trends.operationDurationTemplateName;
const operationSuccessTemplateName = config.stores.trends.operationSuccessTemplateName;
const serviceCountTemplateName = config.stores.trends.serviceCountTemplateName;
const serviceDurationTemplateName = config.stores.trends.serviceDurationTemplateName;
const serviceSuccessTemplateName = config.stores.trends.serviceSuccessTemplateName;
...
function groupByOperation({countValues, successValues, failureValues, tp99Values}) {
    ...
    Object.keys(groupedByOperationName).forEach((operationName) => {
        ...
        if (countPoints.length !== 0) {
            const successPoints = fetchOperationDataPoints(operationTrends, 'success');
            const failurePoints = fetchOperationDataPoints(operationTrends, 'failure');
            ...
            const opKV = {
                ...
                latestTp99Duration: latestTp99DurationDatapoint && latestTp99DurationDatapoint.value / 1000,
                tp99DurationPoints,
                successPoints,
                failurePoints
            };
