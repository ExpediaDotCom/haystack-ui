...
const logger = require('../../../../server/utils/logger').withIdentifier('support:doppler');
...
function serviceStatsFormatter({countValues, successValues, failureValues, tp99Values}) {
    ...
    if (countPoints && countPoints.length !== 0) {
        const successPoints = successValues[0] && successValues[0].success;
        const failurePoints = failureValues[0] && failureValues[0].failure;
        ...
        trendResults.push({
            ...
            totalCount: dataPointsSum(countPoints),
            countPoints,
            avgSuccessPercent: successPoints && failurePoints ? toSuccessPercent(successPoints, failurePoints) : null,
            successPercentPoints: successPoints && failurePoints ? toSuccessPercentPoints(successPoints, failurePoints) : null,
            latestTp99Duration: latestTp99DurationDatapoint && latestTp99DurationDatapoint.value,
            tp99DurationPoints,
            failurePoints
        });
